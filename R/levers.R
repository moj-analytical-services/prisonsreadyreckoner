...
# #' 
# #' @export
# stretch_profiles_old <- function(profiles, lever_profiles_det_stretch_factors, min_stretch_factor, max_stretch_factor, non_data_cols = c("senband", "phase")) {
#   
#   
#   for (non_data_col in non_data_cols) {
#     if (!(non_data_col %in% colnames(profiles)))
...
#       dplyr::filter(senband == !!senband)
#     
#     # Do not stretch profile if stretch_factor = 1; simply return profile
#     if (stretch_factor == 1) {
#       
...
#         dplyr::filter(phase == "pre_impact")
#       
#       input_lag_numbers <- seq(0, ncol(profile_post_impact)-1)
#       
#       # Resample post-impact profile for this senband
#       profile_post_impact_resampled <- mojstockr_resample_profile(input_lag_numbers, profile_post_impact, stretch_factor = stretch_factor) %>%
#         dplyr::mutate(lag_names = paste0("lag", x_resampled), .keep = "unused") %>%
#         tidyr::pivot_wider(names_from = lag_names, values_from = probabilities_resampled) %>%
#         dplyr::bind_cols(profile_post_impact_non_data_cols, .)
#       
#       if (length(profile_post_impact_resampled) > length(profile_pre_impact))
#         profile_post_impact_resampled <- profile_post_impact_resampled[1:length(profile_pre_impact)]
#       
#       # Bind pre- and post-impact profiles together and pad with zeroes as necessary
#       profile_resampled <- dplyr::bind_rows(profile_pre_impact, profile_post_impact_resampled) %>%
#         replace(is.na(.), 0)
#     }
#     
#     
