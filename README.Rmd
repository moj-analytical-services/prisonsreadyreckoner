...
Assuming you are using `renv` for package maanagement, install `prisonsreadyreckoner` by typing the following in the RStudio console:

```r
renv::install("git@github.com:moj-analytical-services/prisonsreadyreckoner.git@v1.0.0")
```

where `v1.0.0` is the latest release. You may wish to double-check there is not
...
Also available at
[our website](https://model-redevelopment-website.apps.alpha.mojanalytics.xyz/).
