...
```yaml
# Parameters used for the formal release of the prisonsreadyreckoner package,
# v2.0.0.
...
# Recall
lever_recall_rate: !expr 'signif(c(senband1 = 0.234774346793349, senband2 = 0.1003465003465, senband3 = 0.048357569686712, senband4 = 0.0213297380452349), 3)'   # 20230505 - XXX - Updated prison projections .msg
...
start_date:
  police_charges_mc: "2022-12-01"    # First month of the forecast. Used for verifying inputs.
  police_charges_cc: "2022-11-01"
  inflows_det: "2023-03-31"
  recall_rate: "2023-03-01"
  cc_files: "2022-11-01"

projection_length_months: 49         # To March 2027. '20230525 - To XXX - Actions from our ready reckoner meeting just now_.msg'
...
```yaml
# Files used for the formal release of the prisonsreadyreckoner package,
# v2.0.0.
# To create a new version, copy into the tmp folder, edit and use
...
# the prisonsreadyreckoner package (v2.0.0).
...
police_charges_mc_files_.mc002:
  description: >-
    ...
    plateaux.
  origin: shiny
  dataFile: "2023-04B/police-charges-mc002-20230825-shiny-v2.0.0-OFFICIAL.csv"
  dataRegister: NA
  comments: Generated with the prisonsreadyreckonerupdater package.
...
main <- function(model_version, scenario_name, scenario_id, params, files, ...) {
  ...
  # Test requested version of the package. Unfortunately, R will not currently
  # allow the same package to be unloaded and loaded in the same session.
  params$version$prisonsreadyreckoner <- sub("^[vV]", "", params$version$prisonsreadyreckoner)
  if (params$version$prisonsreadyreckoner != packageVersion("prisonsreadyreckoner")) {
    stop(
      "Requested prisonsreadyreckoner package version, ",
      params$version$prisonsreadyreckoner,
      ", in scenario, ",
      scenario_name,
      ", is not the same as the loaded package version, ",
      packageVersion("prisonsreadyreckoner"),
      ". ",
      "Start a new session and load the appropriate version."
    )
  }
...
extend_params <- function(params, files) {
  ...
  for (i in 1:length(files)) {
    ...
    # Type 1: Identified with '__', will be represented as a named list.
    # Type 2: Identified with '_.', will be represented as a string vector.
    # Type 3: Identified with none of the above, will be represented as a string.
