...
# run_prisonflowcast()) with respect to levers pulled.
...
test_that("run_prisonflowcast() generates remand forecasts of expected shape and scale when sitting days are added", {
  
  # Background:
  # Our remand population is proportional to the cumulative difference between
  # non-ringfenced receipts and disposals, based on 0.58 IO disposals and 0.277
  # TEW disposals.
  # 
  # Chun-yee's file,
  # s3://alpha-app-criminal-scenario-tool/developmentFolder/Dev_apr23_v1/model_outputs/sensitivity_baseline_230412_0819/crown-output-apr23_s4.csv"
  # indicates around 1,700 remand-inducing non-ringfenced disposals per month.
  
  params <- set_test_params()
  
  silence_botor()
  
  cc_output <- load_crown_output(params$cc_output_file, params$start_date$cc_files, params$forecast_start_date, params$forecast_end_date)
  ...
  # Find what proportion of the original court time was for non-ringfenced cases.  
  non_ringfenced_frac <- dplyr::left_join(cc_output, ringfenced_lookup, by = c("receipt_type_desc", "route")) %>%
                           dplyr::count(ringfenced, wt = dur_disposals)
  non_ringfenced_frac <- non_ringfenced_frac$n[non_ringfenced_frac$ringfenced == FALSE] / sum(non_ringfenced_frac$n)
  
  
  # Apply this fraction to the sitting days used to estimate how many
  # sitting days were devoted to non-ringfenced cases (ignoring the influence of
  # hours per sitting day on sitting day requirement).
  sitting_days_non_ringfenced_base <- non_ringfenced_frac * sum(cc_capacity$sitting_days) / params$projection_length_months

  
  # Sense check:  Add 10% of non-ringfenced sitting days per month and observe
  # the drop in remand population, which should be a linear function with
  # gradient, c. -180 per month. Note, all extra sitting days are for non-
  # ringfenced cases.
  params$lever_extra_cc_sitting_days_impact_date <- params$forecast_start_date
  params$lever_extra_cc_sitting_days <- sitting_days_non_ringfenced_base * 0.1
  pop_combined <- run_prisonflowcast(params)
  #dev_plot_population(pop_combined, "remand", "Remand, 10% more sitting days")

  # Find the extra number of remand cases out-flowing per month
  outflow_remand_delta <- - pop_combined$population[pop_combined$run == "scenario" & pop_combined$casetype == "remand" & pop_combined$sex == "All" & pop_combined$date == params$forecast_end_date] / 
                                params$projection_length_months
  
  # We expect an increase of 10% in non-ringfenced sitting days roughly to
  # increase the outflow of remand cases by 10%.
  outflow_remand_base <- dplyr::left_join(cc_output, ringfenced_lookup, c("receipt_type_desc", "route")) %>%
                           dplyr::filter(receipt_type_desc %in% c("ind", "tew"), ringfenced == FALSE) %>%
                           dplyr::inner_join(params$remand_rates, by = c("receipt_type_desc")) %>%
                           dplyr::mutate(n_disposals_remand = n_disposals * remand_rate)
  outflow_remand_base <- sum(outflow_remand_base$n_disposals_remand) / params$projection_length_months
                      
  expect_equal(round(outflow_remand_delta, -1), round(outflow_remand_base * 0.1, -1))
  
})


test_that("run_prisonflowcast() prison populations respond linearly when sitting days are added", {
  ...
  pop_combined0 <- run_prisonflowcast(params)
  ...
  pop_combined1 <- run_prisonflowcast(params)
...
test_that("run_prisonflowcast() will respond appropriately when additional inflows are added", {
...
test_that("run_prisonflowcast() will respond appropriately when profiles are stretched", {
