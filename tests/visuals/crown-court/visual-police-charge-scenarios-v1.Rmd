---
title: "Police charge scenarios loaded by `prisonsreadyreckoner`"
...
#                   n_disposals_delta = n_receipts_delta) %>%
...
# DEVELOPMENT NOTE: TO KEEP UP TO DATE, TRY SETTING PARAMETERS WITH
# dev_set_params(). See, for example, visual-backlog-clearance.Rmd.
params <- list()
...
`prisonsreadyreckoner`'s ability to model the effect of changes in police charges on prison populations is underpinned by files supplied by David Verry and Katie Mahon from the Upstream and Courts Modelling Team. Here, we visualise these files to understand the different scenarios that they represent.
...
* **April23_mid_pandemic_real_for_ready_reckoner.csv** contains non-zero values throughout. When it is loaded into `prisonsreadyreckoner` using the function `load_police_charges_cc_data_list()`, these non-zero values are set to zero and saved to the list entry `cc_receipts_delta_loaded_list["central"]`. This object provides a default scenario for `prisonsreadyreckoner`. In this default scenario, `prisonsreadyreckoner` uses the same number of Crown Court receipts as the April 2023 central projection. See graph *Central projection (i.e. 24-month ramp)*.
...
<!-- Nothing to plot now fomr the central scenario, which is read in as NA for -->
<!-- efficiency. -->
<!-- ```{r echo=FALSE} -->
...
<!-- * **April23_central_23k_mags_for_ready_reckoner.csv** contains entries with a value of zero throughout. It provides a default scenario for `prisonsreadyreckoner`. In this default scenario, `prisonsreadyreckoner` uses the same number of magistrates' court disposals as the April 2023 central projection. See graph *Central projection (i.e. 24-month ramp)*. -->
