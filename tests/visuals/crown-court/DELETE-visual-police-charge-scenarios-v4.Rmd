---
title: "Police charge comparison. Crown Court disposals with alternative ringfenced table."
author: "Brian Burton"
date: "10/08/2023"
...
In a departure from the assumptions used for prisonsreadyreckoner v1.0.0, we are
assuming that late guilty pleas, late other and other cracked are ringfenced
with a lag indicaed by files provided by the Courts team.
...
* This test supersedes visual-police-charge-scenarios-v3.Rmd, which was
developed on the assumption that late guilty pleas, late other and other cracked
are non-ringfenced. Here we assume they are ring-fenced with a delay chosen from
files provided by the Courts team.
...
* Adjust the ringfenced-lookup table to make sure late guilty pleas, late other
  and other cracked are ringfenced.
  
* Add a lag to all cases, as appropriate.
...
```{r include = FALSE}
  ...
  # Tables of disposals and capacity used in our Crown modelling.
  ...
  # Add a delay to the receipts delta.
    

  cc_data <- load_crown_data(cc_output_file, cc_capacity_file, ringfenced_lookup_file, start_date, forecast_start_date, forecast_end_date)
