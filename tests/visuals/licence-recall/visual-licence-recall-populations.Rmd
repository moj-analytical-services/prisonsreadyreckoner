...
params$projection_length_months <- 79   # Extended to 79 to cover full prison outflow time series.
...
To demonstrate that, when the licence and recall functions of `prisonsreadyreckoner`
...
To compare `prisonsreadyreckoner` to the central projection, we take determinate
prisoner outflows from **File P**. These outflows are then passed to the
`prisonsreadyreckoner` functions responsible for licence and recall to generate a
prediction of the future licence population. This predicted population includes
only individuals who are put on licence after `prisonsreadyreckoner` begins running.
It does not include individuals within the existing licence population. This
predicted population, henceforth referred to as `prisonsreadyreckoner_licence_future`,
is plotted below.
...
plot_series(pop_lic_plot, title = "prisonsreadyreckoner", subtitle = "`prisonsreadyreckoner_licence_future`")

```


The following table compares the steady state values of prisonsreadyreckoner with the
...
plot_series(pop_lic_diff, title = "Difference between `prisonsreadyreckoner` and central projection", subtitle = "`prisonsreadyreckoner_licence_future` - (`licence_forecast_exclPSS` - `licence_basepop_exclPSS`)", ymin = -2500, ymax = 7500)
...
To compare `prisonsreadyreckoner` to the central projection, we pass
`licence_forecast_exclPSS` to the `prisonsreadyreckoner` function
...
recalled after `prisonsreadyreckoner` begins running. It does not include individuals
within the existing recall population.  This predicted population, henceforth
referred to as `prisonsreadyreckoner_recall_future`, is plotted below.
...
plot_series(pop_recall_plot, title = "prisonsreadyreckoner", subtitle = "`prisonsreadyreckoner_recall_future`")
```
...
plot_series(pop_recall_diff, title = "Difference between `prisonsreadyreckoner` and central projection", subtitle = "`prisonsreadyreckoner_recall_future` - `recall_pop_future_calib_exclPSS`")
